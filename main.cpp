...
int main(int argc, char *argv[])
{
    ...
    while((res = pcap_next_ex(handle, &header, &packet))>=0){ //double pointer
        ...
        /* calc size of IP, TCP heaer */
        pEth = (struct ethernet_header *)packet;
        pIph = (struct ip_header *)(packet + sizeof(*pEth));
            ip_size = ((pIph->ip_vhl)&0x0f)*4; // *4 is kind of rule , &0x0f makes bottom 4bit left
        pTcp = (struct tcp_header *)(packet + sizeof(*pEth) + ip_size);
            tcp_size = ((pTcp->th_offx2)>>4)*4;
        ...
        printf("\nlen is : %d\n",len);
        for(i = sizeof(*pEth) + ip_size + tcp_size ; i <= sizeof(*pEth)+len ; i+=16)
        {
            fprintf(stdout, "[%08x] ", i-(sizeof(*pEth) + ip_size + tcp_size));
            for(k = 0 ; k < 16  ; ++k)
            {
                if( k + i < header->len )
